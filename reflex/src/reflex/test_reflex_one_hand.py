...
def main():
    ...
    calibrate_fingers = rospy.ServiceProxy('/reflex_one/calibrate_fingers', Empty)
    ...
    command_pub = rospy.Publisher('/reflex_one/command', Command, queue_size=1)
    pos_pub = rospy.Publisher('/reflex_one/command_position', PoseCommand, queue_size=1)
    vel_pub = rospy.Publisher('/reflex_one/command_velocity', VelocityCommand, queue_size=1)
    force_pub = rospy.Publisher('/reflex_one/command_motor_force', ForceCommand, queue_size=1)
 
    # Constantly capture the current hand state
    rospy.Subscriber('/reflex_one/hand_state', Hand, hand_state_cb)
    ...
    for i in range(10):
        pos_pub.publish(PoseCommand(f1=2.7, f2=2.7, f3=2.7, preshape1=0.0, preshape2=0.0))
        rospy.sleep(1.5)
    ...
    for i in range (10):
        ...
        rospy.sleep(1.0)
        pos_pub.publish(PoseCommand(f1=2.7, f2=2.7, f3=0, preshape1=1.57, preshape2=1.57))
        rospy.sleep(2.0)
        pos_pub.publish(PoseCommand())
        rospy.sleep(2.0)
